...
@app.route(f'{Misc.root}/eula',methods=['GET'])
def eula():
...
@app.route(f'{Misc.root}/network_settings.nws',methods=['GET'])
def network():
...
@app.route(f'{Misc.root}/t_conf',methods=['GET'])
def t_conf():
    return Response("[{\"StatusCode\":200}]",status=200, mimetype='text/plain')

@app.route(f'{Misc.root}/farc_hashes',methods=['GET'])
def hash():
    return Response(status=200, mimetype='text/plain')

@app.route(f'{Misc.root}/notification',methods=['GET'])
def notification():
    return Response("[{\"StatusCode\":200}]",status=200, mimetype='text/plain')

@app.route(f'{Misc.root}/playersInPodCount',methods=['GET'])
...
# <planetStats>
#   <totalSlotCount>10218010</totalSlotCount>
#   <mmPicksCount>991</mmPicksCount>
# </planetStats>
...
def planetStats():
    ...
    xml = Element.createElem("totalSlotCount",totalSlots) + Element.createElem("mmPicksCount",teamPicks)
    return Response(xml,status=200, mimetype='text/xml')



@app.route(f'{Misc.root}/npdata',methods=['POST'])
def npdata():
